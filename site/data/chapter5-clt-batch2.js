...
"113-35":{blanks:[
{a:"I_D=(V_{DD}-V_{DS})/(R_D+R_S)",label:"①",why:"汲極與源極電阻流過同一個直流 \(I_D\)，由電源到接地寫 KVL 可直接求電流。"},
...
{a:"V_G=V_S+V_{GS}",label:"④",why:"由定義 \(V_{GS}=V_G-V_S\) 移項即可得到閘極所需電位。"},
{a:"15kΩ|15kohm|15000ohm",label:"⑤",why:"分壓器必須提供 \(1.0\,\mathrm{V}\) 的閘極電位，反解得到 \(R_G=15\,\mathrm{k}\Omega\)。"}],
...
self:"為什麼本題不能把 \(V_G\) 直接當作 \(V_{GS}\)？請指出源極電阻造成的電位差。",challenge:String.raw`若 \(R_S\) 改為 \(2\,\mathrm{k}\Omega\)，其餘工作點不變，重新設計 \(R_G\)。<br><b>第一步提示：</b>先由 \(V_S=I_DR_S\) 更新源極電位。`},
"113-46":{blanks:[
{a:"V_{GS}>V_t",label:"①",why:"增強型 N 通道 MOSFET 必須先超過臨界電壓才會形成反轉通道。"},
{a:"V_{OV}=V_{GS}-V_t",label:"②",why:"過驅電壓衡量閘源電壓超過臨界值的程度。"},
{a:"V_{DS}≥V_{OV}|VDS>=VOV",label:"③",why:"N 通道增強型 MOSFET 進入飽和區的邊界條件為 \(V_{DS}\ge V_{GS}-V_t\)。"},
{a:"V_{GS}=3V,V_{DS}=1.5V",label:"④",why:"此組數值有 \(V_{OV}=0.5\,\mathrm{V}\)，且 \(1.5\ge0.5\)，符合飽和區。"},
{a:"C",label:"⑤",why:"A、B 的 \(V_{DS}\) 小於 \(V_{OV}\)，D 則未達臨界電壓，只有 C 合格。"}],
...
self:"為什麼 \(V_{DS}\) 很大仍不保證元件導通？請區分『形成通道』與『通道夾止』兩個條件。",challenge:String.raw`若臨界電壓改為 \(V_t=2.0\,\mathrm{V}\)，四個選項中哪些會落在飽和區？<br><b>第一步提示：</b>逐項先算新的 \(V_{OV}=V_{GS}-V_t\)。`},
"113-47":{blanks:[
...
{a:"I_D=K V_{OV}^2",label:"④",why:"題目採用的飽和區模型將係數定義為 \(K\)，不另含 \(1/2\)。"},
{a:"20mA/V^2|20mA/V²",label:"⑤",why:"由 \(K=I_D/V_{OV}^2\) 代入 \(5\,\mathrm{mA}\) 與 \(0.5\,\mathrm{V}\) 得到。"}],
...
self:"同一顆 MOSFET 的 \(K\) 是元件參數；為何改變偏壓後 \(I_D\) 會變而 \(K\) 不應跟著變？",challenge:String.raw`若閘極電壓提高至 \(3.0\,\mathrm{V}\)，用已求得的 \(K\) 預測新的 \(I_D\)。<br><b>第一步提示：</b>先計算新的 \(V_{OV}\)。`},
"114-31":{blanks:[
{a:"導通|on",label:"①",why:"空乏型 MOSFET 在製程中已有通道，因此 \(V_{GS}=0\) 時仍能導通。"},
