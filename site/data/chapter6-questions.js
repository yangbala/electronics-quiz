...
const cropRoot="assets/chapter6/";
...
function solution(){let token=0,limit=q.counts[level-2];return `<div class="steps">${q.steps.map(step=>`<div class="step">${step.replace(/\{\{(.*?)\|(.*?)\}\}/g,(_,answer,unit)=>{token++;if(token>limit)return `<strong>${answer}</strong>${unit==="—"?"":` \\(${unit}\\)`}`;const unitField=unit==="—"?`<span class="dimensionless">無單位</span>`:`<input class="unitInput" data-unit="${unit}" placeholder="${unit}">`;return `<span class="blankRef">${token}</span><span class="answerPair"><input class="blank" data-answer="${answer}" placeholder="數值或文字">${unitField}</span>`})}</div>`).join("")}</div>`}
