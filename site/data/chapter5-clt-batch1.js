...
"111-35":{blanks:[
{a:"V_{SG}=V_S-V_G|VSG=VS-VG",label:"①",why:"P 通道元件以源極為電位參考，改用正值的 \(V_{SG}\) 可減少負號錯誤。"},
{a:"V_{SG}>|V_t|",label:"②",why:"P 通道增強型 MOSFET 必須有足夠的源閘電壓才形成通道。"},
{a:"V_{OV}=V_{SG}-|V_t|",label:"③",why:"過驅電壓代表控制電壓超過臨界值的部分。"},
{a:"V_{SD}<V_{OV}",label:"④",why:"通道尚未在汲極端夾止時屬歐姆區；飽和區需 \(V_{SD}\ge V_{OV}\)。"},
{a:"歐姆區|triode",label:"⑤",why:"實際 \(V_{SD}=0.3\,V\) 遠小於 \(V_{OV}=2.8\,V\)。"}],
model:String.raw`採 P 通道常用的正值電壓定義，控制變數為 {{0}}；形成通道的條件是 {{1}}。`,
derivation:String.raw`<b>步驟 1：求控制電壓與過驅</b>\[V_{SG}=3.3-0=3.3\,\mathrm{V}\]過驅電壓定義為 {{2}}：\[V_{OV}=3.3-0.5=2.8\,\mathrm{V}\]<b>步驟 2：求源汲極電壓</b>\[V_{SD}=V_S-V_D=3.3-3.0=0.3\,\mathrm{V}\]<b>步驟 3：工作區條件</b><br>歐姆區條件為 {{3}}，代入得到 \(0.3<2.8\)。<br><br><b>步驟 4：結論與邊界驗算</b><br>工作區為 {{4}}，選 B；飽和至少需 \(V_{SD}=2.8\,V\)，實際並未達到。`,
self:"為什麼 P 通道題目改用 \(V_{SG}\)、\(V_{SD}\) 能降低符號錯誤？",challenge:String.raw`若汲極電壓降為 \(0.2\,\mathrm{V}\)，其餘不變，判斷工作區。<br><b>第一步提示：</b>重新計算 \(V_{SD}=V_S-V_D\)。`},
"111-48":{blanks:[
{a:"I_D=K(V_{GS}-V_t)^2",label:"①",why:"題目指定飽和區，並採用未含二分之一的 \(K\) 定義。"},
{a:"V_{OV}=V_{GS}-V_t",label:"②",why:"過驅電壓是閘源電壓超過臨界電壓的部分。"},
{a:"g_m=2KV_{OV}",label:"③",why:"轉移電導是 \(I_D\) 對 \(V_{GS}\) 的微分斜率。"},
{a:"8mA/V|8 mA/V",label:"④",why:"代入 \(K\) 與過驅電壓得到工作點小訊號轉移電導。"},
...
self:"\(g_m\) 為什麼是工作點附近的斜率，而不是直流電流本身？",challenge:String.raw`若 \(V_{GS}\) 增加到 \(4\,V\)，求新的 \(g_m\)。<br><b>第一步提示：</b>先重算 \(V_{OV}\)。`},
"112-32":{blanks:[
...
{a:"g_{m0}=2I_{DSS}/|V_P|",label:"②",why:"零偏壓最大轉移電導由 \(I_{DSS}\) 與夾止電壓決定。"},
...
self:"計算 \(1-V_{GS}/V_P\) 時為什麼最容易發生負號錯誤？",challenge:String.raw`若 \(V_{GS}=-2\,V\)，求新的 \(g_m\)。<br><b>第一步提示：</b>先計算 \(1-V_{GS}/V_P\)。`},
"112-34":{blanks:[
...
{a:"V_S=V_G-V_{GS}",label:"④",why:"控制變數定義 \(V_{GS}=V_G-V_S\) 決定源極電位。"},
